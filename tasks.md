...
- [x] **Шаг 1: Базовый каркас.** Создание `.csproj`, манифеста типа Command, реализация команды и пустого немодального WPF-окна. *(Проверка: команда запускается через Add-In Manager, немодальное окно открывается и не блокирует Revit).*
- [x] **Шаг 2: Потокобезопасность.** Реализация и тестирование `IExternalEventHandler`. *(Проверка: тестовая кнопка в WPF асинхронно вызывает чтение ID элемента из Revit без блокировки UI).*
- [x] **Шаг 3: Интеграция LLM.** Подключение к API (например, DeepSeek/OpenAI), реализация чата. *(Проверка: чат стримит текстовые ответы).*
- [x] **Шаг 4: Инфраструктура Skills.** Регистрация базового навыка (например, `GetActiveViewInfo`), передача схемы в LLM. *(Проверка: LLM вызывает навык, ловит ответ, выводит результат в чат).*
- [x] **Шаг 5: Безопасность и запись.** Добавление навыка изменения модели с запросом подтверждения. *(Проверка: LLM пытается изменить параметр, UI запрашивает Approve, после подтверждения транзакция выполняется).*